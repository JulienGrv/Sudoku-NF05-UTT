...
void attenteEcranPrincipal(SDL_Surface *ecran, bool *finDuProgramme)
{
  ...
  // Tant que la fermeture du programme n'a pas �t� demand�e on r�cup�re les
  // �v�nements
  while (*finDuProgramme == false)
  {
    ...
    switch (event.type)
    {
    // Arr�t du programme si la fen�tre est ferm�e
    case SDL_QUIT:
      *finDuProgramme = true;
      break;

    // Cas des touches enfonc�es
    case SDL_KEYDOWN:

      switch (event.key.keysym.sym)
      {
      case SDLK_ESCAPE: // Echap : arr�t du programme
        *finDuProgramme = true;
        break;

      case SDLK_F4: // Alt + F4 : arr�t du programme
    ...
    // Cas des clics de souris rel�ch�s
    case SDL_MOUSEBUTTONUP:

      // Bouton "Nouvelle partie"
      if ((event.button.x >= (ecran->w - 332) / 2) &&
          (event.button.x <= (ecran->w + 332) / 2) && (event.button.y >= 225) &&
          (event.button.y <= 254))
      {
      ...
      else if ((event.button.x >= (ecran->w - 332) / 2) &&
               (event.button.x <= (ecran->w + 332) / 2) &&
               (event.button.y >= 285) &&
               (event.button.y <= 314))
      {
...
void attenteEcranJouer(SDL_Surface *ecran,
                       SDL_Surface *grille,
                       ...
                       bool        *finDuProgramme)
{
  SDL_Event event;
  Uint8    *etatTouche;
  int positionX                 = 0, positionY = 0, i = 0,
      grilleDepart[9][9][10]    = { { { 0 } } }, decalage = 0, colonne = 0,
      ligne                     = 0;
  ...
  while (*finDuProgramme == false)
  {
    ...
    case SDL_MOUSEBUTTONUP:
      ...
      // Clic effectu� sur une case
      if ((event.button.x > (ecran->w - grille->w) / 2) &&
          (event.button.x < (ecran->w + grille->w) / 2) &&
          (event.button.y > (ecran->h - grille->h) / 2) &&
          (event.button.y < (ecran->h + grille->h) / 2))
      {
        // Recherche de la ligne et de la colonne de la case cliqu�e
        ...
        // Proc�dure de saisie d'un chiffre dans la case
        saisirChiffre(ecran, positionX, positionY, finDuProgramme,
                      &(grilleSudoku[ligne][colonne][0]));

        // On efface le cadre rouge
        afficherGrilleVierge2(ecran);

        // On r�-affiche les solutions
      ...
      // Bouton "Valider"
      else if ((event.button.x > ecran->w - 170) &&
               (event.button.x < ecran->w - 20) &&
               (event.button.y > ecran->h - 40) &&
               (event.button.y < ecran->h - 20))
      {
        int niveau = 1, ligne = 0, colonne = 0;

        // On r�sout la grille de d�part
        resolveur(grilleDepart, &niveau);

        // On compare la grille de d�part r�solue � celle propos�e par
        // l'utilisateur
        while (grilleDepart[ligne][colonne][0] ==
               grilleSudoku[ligne][colonne][0] && colonne < 9)
        {
          ligne++;
      ...
      else if ((event.button.x > ecran->w - 340) &&
               (event.button.x < ecran->w - 20) &&
               (event.button.y > ecran->h - 40) &&
               (event.button.y < ecran->h - 20))
      {
...
// grille
void attenteEcranResolveur(SDL_Surface *ecran,
                           SDL_Surface *grille,
                           bool        *finDuProgramme)
{
  SDL_Event event;
...
void saisirChiffre(SDL_Surface *ecran,
                   int          positionX,
                   int          positionY,
                   bool        *finDuProgramme,
                   int         *caseChiffre)
{
  ...
  // On cr�� les surfaces pour le cadre rouge de la case
  SDL_Surface *cadreLargeur = SDL_CreateRGBSurface(SDL_HWSURFACE,
                                                   39,
                                                   3,
                                                   32,
                                                   0,
                                                   ...
                                                   0,
                                                   0);
  SDL_Surface *cadreHauteur = SDL_CreateRGBSurface(SDL_HWSURFACE,
                                                   3,
                                                   39,
                                                   32,
                                                   0,
  ...
  while (numero == 0 && *finDuProgramme == false)
  {
    ...
    case SDL_KEYDOWN:
      ...
      case SDLK_F4:
        ...
        if (etatTouche[SDLK_LALT] && etatTouche[SDLK_F4]) // Alt + F4 : arr�t du
                                                          // programme
        {
          *finDuProgramme = true;
        }
    ...
    // Cas des cliques de souris enfonc�s
