...
bool backtracking(int grille[9][9][10], int position)
{
    ...
    for(k=0; k<9; k++)
    {
        if(absentSurLigne(possibilites[k],grille,i)&&absentSurColonne(possibilites[k],grille,j)&&absentSurBloc(possibilites[k],grille,i,j))
        {
            ...
            // Si le tableau se remplie en entier toutes les cases sont justes et on retourne true
...
/// Fonction qui retourne un tableau rempli dans l'ordre de borneInf inclus à borneSup exclus
int* initSansDoublons(int borneInf, int borneSup)
{
    int taille=borneSup-borneInf, *tableau=malloc((taille)*sizeof(int)), valeur=0;
...
/// Fonction qui retourne un chiffre aléatoire entre borneInf et borneSup exclus
int chiffreAleatoire(int borneInf, int borneSup)
{
    return rand()%(borneSup-borneInf)+borneInf;
}

/// Procédure qui permet de mélanger les valeurs contenu dans un tableau
